...
- **Execute the pre-flight/post-flight checklist for every task.** Before writing code: (1) re-read the full plan once, (2) re-read the task definition, (3) enumerate every sub-task, wire point, file change, and test case into a concrete todo list. After "completing" the task: (1) re-read the plan's task definition, (2) verify every checkbox in the plan, (3) cross-check every wire point has a call site outside tests, (4) verify every deviation was flagged. Do not rely on memory — the todo list is the source of truth.
